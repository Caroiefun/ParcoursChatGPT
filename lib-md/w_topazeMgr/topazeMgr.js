...
var topazeMgr = {
	...
	onLoad : function(){
		...
		} catch(e){console.log("ERROR topazeMgr.onLoad: "+e)}
	},
	...
	/** topazeMgr.addIndexCalc() - Add an auto index calculation.
	 * @param pId : index id
	...
	getIndexVal : function(pId,pType,pDefaultVal){
		...
		switch (pType){
			...
			default: {
				// Doit retourner une valeur numérique par défaut #13318 Changement de calcul des indicateurs dernière version de Topaze ?
				// Cf https://doc.scenari.software/Topaze@1.7/reference/fr/#4_Indicateurs_2
				var vScore = scServices.assmntMgr.getScorePts(pId, this.fSessKey);
				vVl = !isNaN(parseFloat(vScore)) ? vScore : (pDefaultVal !== undefined ? pDefaultVal : (this.isNodeVisited(pId) ? 1 : 0));
	...
	getVarVal : function(pId,pType,pDefaultVal){
		...
		switch (pType){
			...
			default: {
				return scServices.assmntMgr.getResponse(pId, this.fSessKey, "val") || (pDefaultVal!== undefined ? pDefaultVal : "");
			}
		}
	},
