...
var searchMgr = {
	...
	init: function(){
		try {
			...
			console.log("searchMgr.init::Error: "+e);
		}
	},
	...
	onLoad: function(){
		try {
			...
			console.log("searchMgr.onLoad::Error: "+e);
		}
	},
	...
	sReset: function(){
		...
		scCoLib.log("searchMgr.reset");
		scServices.scSearch.resetLastSearch(searchMgr.fIdxUrl);
		searchMgr.xResetUi();
		return false;
	},
	...
	xUpdateUi: function(){
		...
		if (vRes && vRes.length > 0){
			...
			if (!this.fPageList){
				...
				vReq.send();
				this.fPageList = JSON.parse(vReq.responseText);
			}
